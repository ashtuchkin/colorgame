...
Board.prototype.move = function(player_id, x, y) {
    ...
    var count = this.change_color(player.x, player.y, to_color, board2, board_flag);
    
    for (var i = 0; i < this.player_ids.length; i++) {
        var pl = this.players[this.player_ids[i]];
        if (board_flag[pl.y][pl.x] !== (player_id === this.player_ids[i]))
            return {success:false, type:"FILL_OTHER_PLAYER"};
    }
    
    player.count = count;
    this.board = board2;
    return {success:true, changed: board_flag};
}
